...
impl BlockTree {
        ...
        if block_height == current_round {
            for (index_of_tip_ref, tip_ref) in self.current_round_tips_refs.iter().enumerate() {
                if tip_ref.borrow().block.hash.eq(block_hash) {
                    return self.update_recvd_notarization_shares(from_node_number, block_hash, block_height, index_of_tip_ref);
                }
            }
            return false;
    ...
    fn update_recvd_notarization_shares(&mut self, from_node_number: u8, block_hash: &str, block_height: u64, index_of_tip_ref: usize) -> bool {
        Rc::clone(&self.current_round_tips_refs[index_of_tip_ref]).borrow_mut().recvd_notarization_shares[(from_node_number - 1) as usize] = true;
        println!(
            "Block with hash {} has received notarization shares from: {:?}",
            block_hash,
            Rc::clone(&self.current_round_tips_refs[index_of_tip_ref]).borrow().recvd_notarization_shares
        );
        ...
        if !Rc::clone(&self.current_round_tips_refs[index_of_tip_ref]).borrow().is_notarized {
            ...
            if Rc::clone(&self.current_round_tips_refs[index_of_tip_ref]).borrow().recvd_notarization_shares.iter().filter(|&is_received| *is_received == true).count() == N-1 {
                Rc::clone(&self.current_round_tips_refs[index_of_tip_ref]).borrow_mut().is_notarized = true;
